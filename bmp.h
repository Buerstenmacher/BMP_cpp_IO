...
bool operator==(const RGB& b) {return (this->blue == b.blue && this->green == b.green && this->red == b.red)?true:false;}
};//struct RGB

struct DIB_header {
uint32_t header_size = 40; // DIB_header's size is determined by its type. 0x0E/14, 32 bits.
int32_t width; // the bitmap width in pixels (signed integer). 0x12/18, 32 bits.
int32_t height; // the bitmap height in pixels (signed integer). 0x16/22, 32 bits.
...
struct BMP_header {// BMP header's size is fixed, which equals to 14 bits.
char head[3] = { 'B','M','\0' };
uint32_t file_size;
...
int get_row_size(BMP_header h) {return ((h.DIB.bits_per_pixel*h.DIB.width + 31) / 32) * 4;}
int get_data_size(BMP_header h) {return get_row_size(h)*abs(h.DIB.height);}
